...
仓库当前包含黄伟工作群 2026-08-25 发布并已校验的 `公司摄像头主管协作包_20260825_r01`：160 路摄像头、4 台逻辑录像机。4台获批内网 NVR 地址已经作为公开连接目录内置；仓库不保存用户名、密码、原始画面、录像、人员参考图、事件截图或本机运行结果。
...
安装器会自动检查或安装 Git、Node.js 22+、Python 3.11、pnpm 和 Codex，匿名克隆公开脱敏项目到 `%USERPROFILE%\Codex\factory-monitor-workspace-public`，创建独立 `.venv`，安装依赖，执行仓库自检、摄像头目录校验、连接器测试和网页测试。4台地址已内置；若本机尚无 NVR 凭据，安装器只提示普通使用者隐藏录入主厂区与材多的只读用户名和密码，并只保存到当前 Windows 用户的凭据管理器，不需要进入“黄伟工作群”。全部质量门通过后才用 `codex app` 直接打开项目。
...
Windows 从项目内置目录读取地址，从凭据管理器读取本机只读用户名和密码；macOS 从项目内置目录和系统钥匙串读取。指定资料同步人仍可从获批钉钉会话导入，但普通使用者不需要群权限或 `dws`。用户名和密码不会写入仓库。真实回放还要求电脑位于获批的公司网络或 VPN，并拥有对应录像权限。
...
一楼电脑锣六台机开机率使用独立的 `cnc-floor1-runtime-audit` 技能：通道49固定读取1、2、3、5、6号机，通道2“一楼鱼眼1”只补看不到的4号机；每五分钟连续读取10秒并按绿灯闪烁判定，只有1个临界绿帧时自动延长到20秒复核。通道30只用于人员在岗辅助，不能计算开机率；画面移动、机台来源失配或灯位失效时必须返回“待复核”，不能把未检出写成0%。
